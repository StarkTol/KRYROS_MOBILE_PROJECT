             ...
             <div key={it.id} className="rounded-xl border bg-white p-6">
               <div className="flex items-center justify-between">
                 <div>
                   ...
                   <p className="text-lg font-semibold text-slate-900">{it.planName || `${it.months || ""} months`}</p>
                 </div>
               ...
               <div className="mt-4 h-2 w-full rounded-full bg-slate-100">
                 <div
                   ...
                   style={{ width: `${Math.min(100, Math.round(((Number(it.paidAmount) || 0) / Math.max(1, Number(it.amount) || 1)) * 100))}%` }}
                 />
               </div>
               <div className="mt-4 text-xs text-slate-500">
                 Next due: {it.nextDueDate ? new Date(it.nextDueDate).toLocaleDateString() : "—"}
               </div>
             </div>
           ))}
         </div>
